...
{
    //
    public function index(Request $cari)
	{
        $tahun = $cari->tahun? $cari->tahun: date('Y');
        $infrastruktur = $cari->infrastruktur? $cari->infrastruktur: 'semua';
        $pembangunan = $cari->pembangunan? $cari->pembangunan: 'semua';
        $dana = $cari->dana? $cari->dana: 'semua';

        $data = Pembangunan::where('pembangunan_tahun', $tahun)->orWhere('pembangunan_penanggung_jawab', 'like', '%'.$cari.'%')->orWhere('pembangunan_uraian', 'like', '%'.$cari.'%')->orWhere('pembangunan_nilai', 'like', '%'.$cari.'%');

        if ($infrastruktur != 'semua') {
            $data = $data->where('infrastruktur_jenis', $infrastruktur);
            switch ($infrastruktur) {
                case 'Jalan' :
                    $data = $data->orWhereHas('jalan', function($q) use ($cari){
                        $q->where('jalan_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Jembatan' :
                    $data = $data->orWhereHas('jembatan', function($q) use ($cari){
                        $q->where('jembatan_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'DAS' :
                    $data = $data->orWhereHas('das', function($q) use ($cari){
                        $q->where('das_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Drainase' :
                    $data = $data->orWhereHas('drainase', function($q) use ($cari){
                        $q->where('drainase_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'SPAM' :
                    $data = $data->orWhereHas('spam', function($q) use ($cari){
                        $q->where('spam_nama_unit', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Sumur' :
                    $data = $data->orWhereHas('sumur', function($q) use ($cari){
                        $q->where('sumur_kode', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Bendungan' :
                    $data = $data->orWhereHas('bendungan', function($q) use ($cari){
                        $q->where('bendungan_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Daerah Irigasi' :
                    $data = $data->orWhereHas('daerah_irigasi', function($q) use ($cari){
                        $q->where('daerah_irigasi_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Embung' :
                    $data = $data->orWhereHas('embung', function($q) use ($cari){
                        $q->where('embung_nama', 'like', '%'.$cari.'%');
                    });
                break;
                case 'Mata Air' :
                    $data = $data->orWhereHas('mata_air', function($q) use ($cari){
                        $q->where('mata_air_nama', 'like', '%'.$cari.'%');
                    });
                break;
            }
        }
        ...
        try{
            switch ($infrastruktur) {
                case 'Jalan' :
                    ...
                    $infrastruktur->jalan_panjang = $req->get('jalan_panjang');
                ...
                case 'Jembatan' :
                    ...
                    $infrastruktur->jembatan_bangunan_atas_tipe = str_replace(',', '', $req->get('jembatan_bangunan_atas_tipe'));
                    $infrastruktur->jembatan_bangunan_atas_kondisi = str_replace(',', '', $req->get('jembatan_bangunan_atas_kondisi'));
                    $infrastruktur->jembatan_bangunan_bawah_tipe = str_replace(',', '', $req->get('jembatan_bangunan_bawah_tipe'));
                    $infrastruktur->jembatan_bangunan_bawah_kondisi = str_replace(',', '', $req->get('jembatan_bangunan_bawah_kondisi'));
                    $infrastruktur->jembatan_bangunan_pondasi_tipe = str_replace(',', '', $req->get('jembatan_bangunan_pondasi_tipe'));
                    $infrastruktur->jembatan_bangunan_pondasi_kondisi = str_replace(',', '', $req->get('jembatan_bangunan_pondasi_kondisi'));
                    $infrastruktur->jembatan_bangunan_lantai_tipe = str_replace(',', '', $req->get('jembatan_bangunan_lantai_tipe'));
                    $infrastruktur->jembatan_bangunan_lantai_kondisi = str_replace(',', '', $req->get('jembatan_bangunan_lantai_kondisi'));
	...
	public function hapus($id)
	{
		try{
            ...
            $data->delete();
            toast('Berhasil menghapus aspirasi masyarakat '.$data->pembangunan_uraian, 'success')->autoClose(2000);
		}catch(\Exception $e){
